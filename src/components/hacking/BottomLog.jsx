...
export default function BottomLog({ log, selectedNode }) {
  return (
    <div className="flex border-t border-primary/30 bg-background shrink-0" style={{ height: 80 }}>
        ...
        <div className="space-y-0.5 overflow-y-auto h-full">
          {log.length === 0 ? (
            ...
            log.slice(0, 6).map((entry, i) => (
              <p key={i} className={cn('font-mono text-[10px]', TYPE_COLOR[entry.type] || 'text-primary/60')}>
                &gt; {entry.text}
              </p>
            ))
          )}
      ...
      <div className="flex-1 px-3 py-2 overflow-hidden">
        {selectedNode ? (
          <div className="space-y-0.5">
            ...
            <p className="font-mono text-[11px] text-primary">{selectedNode.name}</p>
            <p className="font-mono text-[10px] text-primary/60">DC {selectedNode.dc} · {selectedNode.successes_current || 0}/{selectedNode.successes_required || 0} successes</p>
          </div>
        ) : (
